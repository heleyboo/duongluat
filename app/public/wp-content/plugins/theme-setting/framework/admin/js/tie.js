jQuery(document).ready(function() {
    jQuery('.ts-tooltip').tipsy({fade: true, gravity: 's'});

	if( jQuery('li.menu-top.toplevel_page_panel .ts-theme-update').length ){
	...
	jQuery("select[name='tie_options[typography_test][font]']").change(function(){
		...
		var output =  parts[0] ;
		jQuery("#ts-fonts-css").attr( 'href' , 'https://fonts.googleapis.com/css?family='+output );
...
// Single page template Head
	...
	if (selected_item == 'template-blog.php' || selected_item == 'template-masonry.php' || selected_item == 'template-media.php' ) {jQuery('#ts-template-blog').show();}
	if (selected_item == 'template-best-reviews.php') {jQuery('#ts-template-blog, #tie_posts_num-item').show();}
	if (selected_item == 'template-authors.php') {jQuery('#ts-template-authors').show();}

	jQuery("select[name='page_template']").change(function(){
		var selected_item = jQuery("select[name='page_template'] option:selected").val();
		jQuery('#ts-template-blog, #ts-template-authors, #tie_posts_num-item').hide();

		if (selected_item == 'template-blog.php' || selected_item == 'template-masonry.php' || selected_item == 'template-media.php' ) {
			jQuery('#ts-template-blog').fadeIn();
		}
		if (selected_item == 'template-best-reviews.php') {
			jQuery('#ts-template-blog, #tie_posts_num-item').fadeIn();
		}
		if (selected_item == 'template-authors.php') {
			jQuery('#ts-template-authors').fadeIn();
		}
	 });
...
-------------------------------------------------------------------------------- */
	...
	jQuery("#tie_page_builder").click( function() {
		...
		if( jQuery(this).hasClass( 'builder_active' ) ){
			...
			jQuery( '#postdivrich, #pageparentdiv, #postimagediv, #tie_post_head_options_box, #tie_ads_options_box, #ts-template-authors, #ts-template-blog, #tie_sidebar_position_full' ).hide();
...
// Panel Tabs
	...
	jQuery("li.ts-tabs:not(.ts-not-tab)").click(function() {
...
// Ctageories options
	jQuery(".ts-cats-options input:checked").parent().addClass("selected");
	jQuery(document).on("click", ".ts-cats-options .checkbox-select" , function(event){
...
// Ctageories Tabs box
	...
	var allPanels = jQuery('.ts-accordion > .ts-accordion-contnet').hide();
	jQuery('.ts-accordion > .accordion-head > a').click(function() {
		$this = jQuery(this);
	...
	jQuery( 'ul.ts-options:not(.ts-cats-options)' ).each(function( index ) {
	...
	jQuery(document).ready(function() {
		...
		var $sidebar = jQuery(".mo-panel-tabs");
		var $sidebarHeight = $sidebar.innerHeight();
		var $footerOffsetTop = jQuery("#wpfooter").offset().top;
		var $sidebarOffset = $sidebar.offset();

		$window.scroll(function() {
			if($sidebarOffset && $window.scrollTop() > $sidebarOffset.top) {
				$sidebar.css('position', 'fixed');
				$sidebar.css('top', $sidebarOffset.top);
...
// image Uploader Functions
	function ts_set_uploader(field, styling ) {
		var tie_bg_uploader;
